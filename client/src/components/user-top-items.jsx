...
export default function UserTopItems({ type, title }) {
    ...
    useEffect(() => {
        ...
        async function fetchTopItems() {
            ...
            const res = await getUserTopItems(userCredentials.accessToken, type)
            setTopItems(res)
        }
    ...
    return (
        <>
            ...
            <div className={type === 'tracks' ? "row-elements light-background" : 'row-elements'} ref={containerRef}>
                {!isMobile && scroll != 0 ?
                    <i class='fas fa-angle-left scroll-arrow' onClick={() => {
                        containerRef.current?.scrollBy({ left: -400, behavior: "smooth" })
                    }}></i>
                    : null
                }
                {topItems === undefined ? <Loading /> : topItems.items.map(item => {
                ...
                {!isMobile && containerRef.current?.clientWidth + scroll < containerRef.current?.scrollWidth ?
                    <i class='fas fa-angle-right scroll-arrow right-arrow'
                        onClick={() => {
                            containerRef.current?.scrollBy({ left: 400, behavior: "smooth" })
                        }}
                    ></i>
